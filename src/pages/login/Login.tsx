...
import { getProfileLS } from 'src/utils/auth'

const schemaAuthLogin = schemaAuth.pick(['email', 'password'])
type SchemaAuthLogin = Pick<SchemaAuth, 'email' | 'password'>
const Login = () => {
  ...
  const {
    ...
    setError,
    formState: { errors }
  } = useForm<SchemaAuthLogin>({
    resolver: yupResolver(schemaAuthLogin)
  })

  const loginAccountMutation = useMutation({
    mutationFn: (body: SchemaAuthLogin) => authApi.login(body)
  })

  const onSubmit: SubmitHandler<SchemaAuthLogin> = (data) => {
    loginAccountMutation.mutate(data, {
      onSuccess: () => {
        setIsAuthenticated(true)
        setProfile(getProfileLS())
        nav('/')
      },
      onError: (error) => {
        if (isAxiosUnprocessableEntityError<ErrorResponse<SchemaAuthLogin>>(error)) {
        ...
        <div className=' grid h-full items-center p-5 lg:grid-cols-12'>
          <div className='bg-white p-10 lg:col-span-12 lg:col-start-8'>
            ...
            <form className='mt-7 flex flex-col gap-1' onSubmit={handleSubmit(onSubmit)}>
              ...
              <button
                ...
                className={`bg-red-500 py-3 font-semibold text-white shadow-md ${
                  loginAccountMutation.isLoading ? 'flex items-center justify-center' : null
                }`}
              >
            ...
            <p className='mt-6 text-center text-xl'>
              {' '}
              Bạn đã có tài khoản?
              <Link to='/register' className='ml-2 text-2xl text-red-500'>
                Đăng ký
              </Link>
            </p>
          </div>
